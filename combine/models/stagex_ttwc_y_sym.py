...
#y0 = (-1.177641)
class stagex_ttwc_y_sym(PhysicsModel):
	...
	def getYieldScale(self,bin,process):
		...
		if self.acttbar:
			if not self.acmu:
				...
				self.modelBuilder.factory_("expr::r_tt_times_ci0100(\"0.298038*(@0+1.177641)*(@0+1.177641)-0.41333\", y)")
				self.modelBuilder.factory_("expr::r_tt_times_ci0000(\"-0.298038*(@0+1.177641)*(@0+1.177641)+1.41333\", y)")
				self.pois.append("y")
			else:
				self.modelBuilder.doVar("y[0.0,-5,5]" )
				self.modelBuilder.doVar("mu[1.0,0,10]" )
				self.modelBuilder.factory_("expr::r_tt_times_ci0100(\"0.298038*(@0+1.177641)*(@0+1.177641)-0.41333*@1\", y, mu)")
				self.modelBuilder.factory_("expr::r_tt_times_ci0000(\"-0.298038*(@0+1.177641)*(@0+1.177641)+1+0.41333*@1\", y, mu)")
				self.modelBuilder.factory_("expr::r_tt_times_ci0010(\"-1+@0\", mu)")
				self.pois.append("y")
...
stagex_ttwc_y_sym = stagex_ttwc_y_sym()