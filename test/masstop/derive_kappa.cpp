#include<TROOT.h>
#include<TStyle.h>
#include<Math/PdfFuncMathCore.h>
#include<TFile.h>
#include<TChain.h>
#include<TLorentzVector.h>
#include<TMath.h>
#include<TLegend.h>
#include<TF1.h>
#include<TH1D.h>
#include<TCanvas.h>
#include<TH2F.h>
#include<THStack.h>
#include<string.h>
#include<fstream>
#include<iostream>
#include<TGraph.h>
using namespace std;
double p2weight=0.65/0.3;
void renew_weight(TString* weight, TString file, int j, TString path){
    const int nsample = 3;
    Float_t cross_sections[nsample]={366.91, 89.05, 377.96};
    Float_t K_Factor[nsample]={1.0, 1.0, 1.0,};
    float lumi=59.83;
    auto c1 = new TCanvas("c0", "c0", 8, 30, 600, 600); // temporary canvas
    TChain* tree1=new TChain("rawtree"); 
    tree1->Add(path+"MC/"+file);
    TH1D *nmc=new TH1D("nmc","",50,0,100);
    nmc->Sumw2();
    c1->cd();
    tree1->Draw("nJet>>nmc","Generator_weight");
    //cout<<nmc->GetSumOfWeights()<<endl;
    float weight1=cross_sections[j]*lumi/(nmc->GetSumOfWeights())*K_Factor[j]*1000;
    //cout<<weight1<<endl;
    *weight = Form("%f*", weight1) + (*weight);
    delete tree1;
    delete nmc;
    delete c1;
}
void get_num( TString other_con1, TString other_con2, int bin, double value[3][2]){
    const int nsample = 3;
    TString fileNames[][nsample] = {{  
                            "new_TTToSemiLeptonic_TuneCP5_13TeV-powheg.root",
                            "new_TTTo2L2Nu_TuneCP5_13TeV-powheg.root",
                            "new_TTToHadronic_TuneCP5_13TeV-powheg.root"
                            },
                            {  
                            "new_TTToSemiLeptonic_mtop171p5_TuneCP5_13TeV-powheg.root",
                            "new_TTTo2L2Nu_mtop171p5_TuneCP5_13TeV-powheg.root",
                            "new_TTToHadronic_mtop171p5_TuneCP5_13TeV-powheg.root"
                            },
                            {  
                            "new_TTToSemiLeptonic_mtop173p5_TuneCP5_13TeV-powheg.root",
                            "new_TTTo2L2Nu_mtop173p5_TuneCP5_13TeV-powheg.root",
                            "new_TTToHadronic_mtop173p5_TuneCP5_13TeV-powheg.root"
                            }};
    for(int n=0; n<3; n++){
        for(int i=0;i<nsample;i++)
            fileNames[n][i]=fileNames[n][i].ReplaceAll(".root","_*.root");
    }
    TString file_up, file_down; 
    TString title = "Mtt";
    TString xvars = "mass_tt";
    TString w_mtt[2] = {"mtt_ci0000", "mtt_ci0010"};
    TString weight_up, weight_down;
    TH1D *hist0, *hist1;
    TH1D *hmc0, *hmc1;
    TString path = "./";
    //TFile* file = new TFile(path+file_name, "update");
    double xbins[] = {0, 330, 350, 370, 400, 450, 500, 600, 700, 1000, 2000};
    int nbins = 10;
    for(int i=0; i<2; i++){
        for(int n=0; n<3; n++){
            for(int j=0; j<3; j++){
                TString weight = "Generator_weight*SF_btag*SF_lepton*L1PreFiringWeight_Nom*nnlo_wt"+other_con1+other_con2;
                TString weight1 = "Generator_weight*SF_btag*SF_lepton*L1PreFiringWeight_Nom*nnlo_wt*"+w_mtt[i]+other_con1+other_con2;
                TChain* tree=new TChain("mytree");
                hist1 = new TH1D("CG1","", nbins, xbins);
                hist0 = new TH1D("CG","", nbins, xbins);
                hist0->Sumw2();
                hist1->Sumw2();
                renew_weight(&weight, fileNames[n][j], j, path);
                renew_weight(&weight1, fileNames[n][j], j, path);
                tree->Add(path+"MC/"+fileNames[n][j]);
                tree->Draw(xvars+">>CG", weight);
                tree->Draw(xvars+">>CG1", weight1);
                //cout<<hist0->GetBinContent(bin)<<" "<<hist1->GetBinContent(bin)<<endl;
                //cout<<hist1->GetSumOfWeights()<<" "<<hist2->GetSumOfWeights()<<endl;
                if(j==0){
                    hmc0 = (TH1D*)hist0->Clone();
                    hmc0->SetName(title);
                    hmc1 = (TH1D*)hist1->Clone();
                    hmc1->SetName(title+"_ew");
                }
                else{
                    hmc0->Add(hist0);
                    hmc1->Add(hist1);
                } 
                delete hist0;             
                delete tree;
                delete hist1;
            }
            value[n][i] = hmc1->GetBinContent(bin)/hmc0->GetBinContent(bin);
            delete hmc0;
            delete hmc1; 
        }
    }
}
void derive_kappa(){
    TString other_con1[] = {"*(jet_num>=4)", "*(jet_num==3 && jet_pt[0]>50)"};
    TString other_con2[] = {"*lep_flavour", "*((!lep_flavour) && (lepton_pt>34))"};
    TString other_con3 = "*(jet_num>=4 || (jet_num==3 && jet_pt[0]>50))";
    TString other_con4 = "*(lep_flavour || ((!lep_flavour) && lepton_pt>34))";
    TString lepton[] ={"_M", "_E"};
    TString jet[] = {"_4jets", "_3jets"};
    TGraph *h[3];
    double kappa[] = {0, 0.5, 1.0, 1.5, 2.0, 2.5};
    //draw_pre(2018, other_con3, other_con4, "EWun.root");
    TFile* file = new TFile("./EW_mtop.root", "recreate");
    double value_kappa[3];
    double value[3][2];
    
    TString name[3] = {"EW_kappa_mt172", "EW_kappa_mt171", "EW_kappa_mt173"};
    for(int n=0; n<5; n++){
        for(int j=0; j<3; j++)
            h[j] = new TGraph();
        get_num(other_con1[0], other_con2[0], n+1, value);
        for(int i=0; i<6; i++){
            for(int j=0; j<3; j++){
                value_kappa[j] = (1 - kappa[i] * kappa[i]) * value[j][1] + kappa[i] * kappa[i] * value[j][0];
                h[j]->SetPoint(i, kappa[i], value_kappa[j]);
            }
        }
        file->cd();
        for(int j=0; j<3; j++){
            h[j]->GetXaxis()->SetRangeUser(-0.25, 2.75);
            h[j]->GetYaxis()->SetRangeUser(0.90, 1.25);
            h[j]->SetName(name[j]+Form("_bin%d", n+1));
            h[j]->Write();
            delete h[j];
        }
    }
    file->Close();
}