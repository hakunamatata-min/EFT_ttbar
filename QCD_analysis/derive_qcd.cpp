...
void derive(TString cut, TString cut_name, int l, int g){
    ...
    TH1D *hist1, *hist2;
    TFile* file = new TFile("./QCD/QCD_"+cut_name+"_"+cg+".root", "recreate");
    ...
    hist1 = new TH1D("hist1", "", nbins[0], xbins[0]);
    hist2 = new TH1D("hist2", "", nbins[1], xbins[1]);
    hdata = new TH1D("QCD", "", bin_num, 0, bin_num);
    c1->cd();
    tree2->Draw("mass_tt>>hist1", cut+other_con1+other_con2+div_con[0]);
    tree2->Draw("mass_tt>>hist2", cut+other_con1+other_con2+div_con[1]);
    sum_h1(hdata, hist1, hist2);
    ...
    for(int k=0; k<2; k++){
        for(int j=edge[k]; j<edge[k+1]; j++){
            ...
            hist1 = new TH1D("hist1", "", nbins[0], xbins[0]);
            hist2 = new TH1D("hist2", "", nbins[1], xbins[1]);
            TH1D* hist = new TH1D("hist", "", bin_num, 0, bin_num);
            hist1->Sumw2(); hist2->Sumw2();
            tree_b[j]->Draw("mass_tt>>hist1", weight+div_con[0]);
            tree_b[j]->Draw("mass_tt>>hist2", weight+div_con[1]);
            sum_h1(hist, hist1, hist2);
            delete hist1; delete hist2;
            if(j==edge[k]){
    ...
    for(int j=edge[1]; j<edge[2]; j++){
        ...
        TH1D* hist = new TH1D("hist", "", bin_num, 0, bin_num);
        hist1->Sumw2(); hist2->Sumw2();
        tree_a[j]->Draw("mass_tt>>hist1", weight+div_con[0]);
        tree_a[j]->Draw("mass_tt>>hist2", weight+div_con[1]);
        sum_h1(hist, hist1, hist2);
        delete hist1; delete hist2;
        if(j==edge[1]){
...
void derive_qcd(){
    ...
    for(int g=1; g<4; g++){
        for(int i=0; i<4; i++){
            derive(cuts[i], cutsName[i], lep[i], g);
        }
    }
