...
#y0 = (-1.177641)
class stagex_ttwc_z_sym(PhysicsModel):
	...
	def getYieldScale(self,bin,process):
		...
		if self.stage0:
			if "ttbar" in process:
				if self.acttbar:
					...
					elif self.acmu and "ci0100" in process:
		...
		if self.acttbar:
			if not self.acmu:
				...
				self.modelBuilder.factory_("expr::r_tt_times_ci0000(\"(@0-1)*(@0-1)-0.41333+0.41333*@1\", z, mu)")
				self.modelBuilder.factory_("expr::r_tt_times_ci0100(\"0.41333-0.41333*@0\", mu)")
				self.pois.append("z")
...
stagex_ttwc_z_sym = stagex_ttwc_z_sym()
