...
using namespace std;
BTagCalibration_offReader reader(BTagEntry_off::OP_RESHAPING, "central", {"up_lf","down_lf","up_hf","down_hf","up_cferr1", "down_cferr1"});
class SF_add_it_tree{
private:
    ...
    BTagCalibration_offReader* reader;
    ...
    void sf_btag(BTagEntry_off::JetFlavor flav, Float_t pt, Float_t eta, Float_t disc, Float_t sf[3]);
    ...
    void set_dir(int year);
public:
    SF_add_it_tree(TString inputFile, TString tree_name, bool remain_sys, int year);
    ~SF_add_it_tree();
};