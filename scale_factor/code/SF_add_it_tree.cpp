#include"SF_add_it_tree.h"
void SF_add_it_tree::get_string1(Float_t pt, Float_t eta, TString s[2]){
...
void SF_add_it_tree::get_string2(Float_t eta, TString s[2]){
...
void SF_add_it_tree::sf_electronreco(Float_t pt, Float_t eta, Float_t weight[3]){
    Int_t nbin;
    if(pt>500.0 || eta>2.5){
        weight[0]*=1.0;
        weight[1]*=1.0;
...
void SF_add_it_tree::sf_electronid(Float_t pt, Float_t eta, Float_t weight[3]){
    Int_t nbin;
    if(pt>500.0 || eta>2.5){
        weight[0]*=1.0;
        weight[1]*=1.0;
...
void SF_add_it_tree::sf_muoniso(Float_t pt, Float_t eta, Float_t weight[3]){
...
void SF_add_it_tree::sf_muonid(Float_t pt, Float_t eta, Float_t weight[3]){
...
void SF_add_it_tree::sf_muonreco(Float_t pt, Float_t eta, Float_t weight[3]){
    TString s[2];
    float nom;
...
void SF_add_it_tree::sf_btag(BTagEntry_off::JetFlavor flav, Float_t pt, Float_t eta, Float_t disc, Float_t sf[3]){
    sf[0] = reader->eval_auto_bounds("central",flav, eta, pt, disc);
    if(flav==BTagEntry_off::FLAV_B){
        sf[1] = reader->eval_auto_bounds("up_hf", flav, eta, pt, disc);
        sf[2] = reader->eval_auto_bounds("down_hf", flav, eta, pt, disc);
    }
    else if(flav==BTagEntry_off::FLAV_C){
        sf[1] = reader->eval_auto_bounds("up_cferr1", flav, eta, pt, disc);
        sf[2] = reader->eval_auto_bounds("down_cferr1", flav, eta, pt, disc);
    }
    else{
        sf[1] = reader->eval_auto_bounds("up_lf", flav, eta, pt, disc);
        sf[2] = reader->eval_auto_bounds("down_lf", flav, eta, pt, disc);
    }
}
Float_t SF_add_it_tree::btag_eff(Int_t flav, Float_t pt, Float_t eta){
    Int_t bin;
    if(flav==5 || flav==-5){
...
void SF_add_it_tree::sf_lep(Float_t pt, Float_t eta, Bool_t flavour, Float_t weight[3]){
...
void SF_add_it_tree::sf_jet(Float_t *pt, Float_t *eta, Int_t *flavour, Float_t* score, UInt_t jet_num, Float_t weight[3]){
    Float_t mc=1;
    Float_t sf[3],data[3];
    for(int i=0; i<3; i++)
        data[i]=1;
    ...
    for(int i=0; i<jet_num; i++){
        ...
        sf_btag(flav, pt[i], fabs(eta[i]), score[i] ,sf);
        //cout<<sf[2]<<" "<<sf[0]<<" "<<sf[1]<<endl;
        mc *= btag_eff(flavour[i], pt[i], fabs(eta[i]));
        data[0] *= btag_eff(flavour[i], pt[i], fabs(eta[i]))*sf[0];
        data[1] *= btag_eff(flavour[i], pt[i], fabs(eta[i]))*sf[1];
        data[2] *= btag_eff(flavour[i], pt[i], fabs(eta[i]))*sf[2];
        cout<<sf[0]<<" ";
    }
    cout<<endl;
    weight[0] = data[0]/mc;
...
void SF_add_it_tree::set_dir(int year){
    ...
    if(year == 2018){
        ...
        bfile = indir+"/"+sf_dir+"/btag/reshaping_deepJet_106XUL18_v2.csv";
    }
    else if(year == 2017){
        ...
        muon_m_reco = "NUM_TrackerMuons_DEN_genTracks_Z_abseta_pt.json";
        bfile = indir+"/"+sf_dir+"/btag/reshaping_deepJet_106XUL17.csv";
    }
        ...
        muon_m_reco = "NUM_TrackerMuons_DEN_genTracks_Z_abseta_pt.json";
        bfile=indir+"/"+sf_dir+"/btag/reshaping_deepJet_106XUL16postVFP_v3.csv";
    }
    else if(year == 2015){//2016_pre
        //indir="/home/yksong/code/ttbar/scale_factor/2016_pre";
        sf_dir="UL2016_pre";
        ...
        muon_m_reco = "NUM_TrackerMuons_DEN_genTracks_Z_abseta_pt.json";
        bfile=indir+"/"+sf_dir+"/btag/reshaping_deepJet_106XUL16preVFP.csv";
    }
}
SF_add_it_tree::SF_add_it_tree(TString inputFile, TString tree_name, bool remain_sys, int year){
    ...
    Float_t sf_lepton_up, sf_lepton_down, sf_btag_up, sf_btag_down; 
    TBranch* newbranch[6];
    newbranch[0]=mytree->Branch("SF_lepton",&sf_lepton,"SF_lepton/F");
    newbranch[1]=mytree->Branch("SF_btag",&sf_btag,"SF_btag/F");
    if(remain_sys){
        newbranch[2]=mytree->Branch("SF_lepton_up", &sf_lepton_up, "sf_lepton_up/F");
        newbranch[3]=mytree->Branch("SF_lepton_down", &sf_lepton_down, "sf_lepton_down/F");
        newbranch[4]=mytree->Branch("SF_btag_up", &sf_btag_up, "sf_btag_up/F");
        newbranch[5]=mytree->Branch("SF_btag_down", &sf_btag_down, "sf_btag_down/F");
    }
    ...
    reader = new BTagCalibration_offReader(BTagEntry_off::OP_RESHAPING, "central", {"up_lf","down_lf","up_hf","down_hf","up_cferr1", "down_cferr1"});
    reader->load(calib, BTagEntry_off::FLAV_B, "iterativefit");
    reader->load(calib, BTagEntry_off::FLAV_C, "iterativefit");
    reader->load(calib, BTagEntry_off::FLAV_UDSG, "iterativefit");
    
    Float_t weight_jet[3], weight_lep[3]; 
    for(int entry=0;entry<mytree->GetEntries();entry++){
        ...
        //cout<<weight_jet[0]<<endl;
        sf_lepton = weight_lep[0];
        sf_btag = weight_jet[0];
        for(int nbra=0; nbra<2; nbra++)
            newbranch[nbra]->Fill();
        if(remain_sys){
...
SF_add_it_tree::~SF_add_it_tree(){
    delete reader;
}
int main(){
    SF_add_it_tree *s;
    s = new SF_add_it_tree("./ttbar/select/test/new_TTsemi.root", "mytree", true, 2018);
    delete s;
}