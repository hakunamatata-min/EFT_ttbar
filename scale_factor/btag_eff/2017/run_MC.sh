...
echo "output: $output"
wrong="f"
cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/scale_factor/btag_eff/2017/condor_out_MC/$1
file=$(ls ${1}.txt)
dir_f=$(cat $file)
#dir="root://cms-xrd-global.cern.ch/"$dir
dir="root://xrootd-cms.infn.it/"$dir_f
eos="/eos/user/y/yuekai/ttbar/btag_eff/2017/"
inputFile=${file%.txt*}
...
mv $output/out*.txt /afs/cern.ch/user/y/yuekai/EFT-ttbar/scale_factor/btag_eff/2017/condor_out_MC/$1
if [[ $wrong == "f" ]]
then
    input=$(ls $output/*root)
    cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/scale_factor/btag_eff
    root -l -q -b pre_eff.cpp"(\"$output\",\"$inputFile\",\"$output/$input\", 2017)";
