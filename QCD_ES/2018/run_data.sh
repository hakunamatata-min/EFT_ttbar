...
eos="/eos/user/y/yuekai/ttbar/QCD_ES/2018/data"
inputFile=${file%.txt*}
...
then
    ...
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2018,2,0)"
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2018,3,0)"
