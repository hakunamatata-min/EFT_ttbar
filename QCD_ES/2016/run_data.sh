...
echo "output: $output"
wrong="f"
cd /afs/cern.ch/user/y/yuekai/ttbar/condor/QCD_ES/2016/condor_out_data/$1
file=$(ls ${1}.txt)
dir_f=$(cat $file)
#dir="root://cms-xrd-global.cern.ch/"$dir
dir="root://xrootd-cms.infn.it/"$dir_f
eos="/eos/user/y/yuekai/ttbar/QCD_ES/2016/data"
inputFile=${file%.txt*}
...
mv $output/out*.txt /afs/cern.ch/user/y/yuekai/ttbar/condor/QCD_ES/2016/condor_out_data/$1
if [[ $wrong == "f" ]]
then
    echo "input file: $dir"
    cd /afs/cern.ch/user/y/yuekai/ttbar/condor/QCD_ES
    input=$(ls $output|grep root)
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2016,0,0)"
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2016,1,0)"
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2016,2,0)"
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2016,3,0)"
