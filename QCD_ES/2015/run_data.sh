...
echo "output: $output"
wrong="f"
cd /afs/cern.ch/user/y/yuekai/ttbar/condor/QCD_ES/2015/condor_out_data/$1
file=$(ls ${1}.txt)
dir_f=$(cat $file)
#dir="root://cms-xrd-global.cern.ch/"$dir
dir="root://xrootd-cms.infn.it/"$dir_f
eos="/eos/user/y/yuekai/ttbar/QCD_ES/2015/data"
inputFile=${file%.txt*}
...
mv $output/out*.txt /afs/cern.ch/user/y/yuekai/ttbar/condor/QCD_ES/2015/condor_out_data/$1
if [[ $wrong == "f" ]]
then
    echo "input file: $dir"
    cd /afs/cern.ch/user/y/yuekai/ttbar/condor/QCD_ES
    input=$(ls $output|grep root)
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2015,0,0)"
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2015,1,0)"
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2015,2,0)"
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2015,3,0)"
