...
echo "output: $output"
wrong="f"
cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/2017/condor_out_MC/$1
file=$(ls ${1}.txt)
dir_f=$(cat $file)
#dir="root://cms-xrd-global.cern.ch/"$dir
dir="root://xrootd-cms.infn.it/"$dir_f
eos="/eos/user/y/yuekai/ttbar/select/2017/MC"
inputFile=${file%.txt*}
...
mv $output/out*.txt /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/2017/condor_out_MC/$1
if [[ $wrong == "f" ]]
then
    echo "input file: $dir"
    ...
    do
        root -l -q -b ./SF_add.cpp"(\"$outputFile\",0,2017)"
    ...
    then
        ...
        for outputFile in $(ls $output/new*.root)
        do
            root -l -q -b ./nnlo_add.cpp"(\"$outputFile\",0,2017)"
        done
        cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/EW_weight
        for outputFile in $(ls $output/new*.root)
        do
            root -l -q -b ./add_weight_branch.cpp"(\"$outputFile\",0,2017)"
        done
    ...
    for outputFile in $(ls $output/new*.root)
    do
        root -l -q -b add_pu.cpp"(\"$outputFile\",2017,0)"
    done
