...
echo "output: $output"
wrong="f"
cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/2017/condor_out_sys/$1
file=$(ls ${1}.txt)
dir_f=$(cat $file)
...
mv $output/out*.txt /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/2017/condor_out_sys/$1
if [[ $wrong == "f" ]]
then
    echo "input file: $dir"
    ...
    then
        ...
        for outputFile in $(ls $output/new*.root)
        do
            root -l -q -b ./nnlo_add.cpp"(\"$outputFile\",1,2017)"
        done
        cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/EW_weight
        for outputFile in $(ls $output/new*.root)
        do
            root -l -q -b ./add_weight_branch.cpp"(\"$outputFile\",1,2017)"
        done
    fi
