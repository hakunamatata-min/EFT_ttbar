...
echo "output: $output"
wrong="f"
cd /afs/cern.ch/user/y/yuekai/ttbar/condor/select/2018/condor_out_mtop/$1
file=$(ls ${1}.txt)
dir_f=$(cat $file)
#dir="root://cms-xrd-global.cern.ch/"$dir
dir="root://xrootd-cms.infn.it/"$dir_f
eos="/eos/user/y/yuekai/ttbar/select/2018/MC_sys"
inputFile=${file%.txt*}
...
mv $output/out*.txt /afs/cern.ch/user/y/yuekai/ttbar/condor/select/2018/condor_out_mtop/$1
if [[ $wrong == "f" ]]
then
    echo "input file: $dir"
    ...
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2018,2)"
